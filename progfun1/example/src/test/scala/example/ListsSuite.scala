...
class ListsSuite {
  ...
  @Test def `sum of single element list`: Unit = {
    assertEquals(2, sum(List(2)))
  }

  @Test def `sum of a few negative numbers`: Unit = {
    assertEquals(-9, sum(List(-3, -1, -1, -4, 0)))
    }
  ...
  @Test def `max of single element list`: Unit = {
    assertEquals(2, max(List(2)))
  }

  @Test def `max of a few repeating numbers`: Unit = {
    assertEquals(5, max(List(2, 3, 3, 0, 0, 2, 5, 5)))
  }

  @Test def `max of a few negative numbers`: Unit = {
    assertEquals(-1, max(List(-3, -10, -1, -12, -4)))
  }
