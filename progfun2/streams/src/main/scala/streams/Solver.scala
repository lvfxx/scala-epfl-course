...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList()
      ...
      val nextPaths = newNeighborsOnly(neighborsWithHistory(block, history), explored ++ (initial map (_._1)))
      initial.head #:: from(initial.tail #::: nextPaths, explored + initial.head._1)
    }
  }
