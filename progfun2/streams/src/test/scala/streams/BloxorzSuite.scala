...
class BloxorzSuite {
  ...
  @Test def `neighbours with history`: Unit =
    new Level1 {
      assertEquals(
        Set(
  ...
  @Test def `new neighbours only`: Unit =
    new Level1 {
      val neighboursOnly = newNeighborsOnly(
        Set(
          (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
          (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
        ).to(LazyList),
      ...
      assertEquals(Set((Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))).to(LazyList), neighboursOnly)
    }
